...
const createCategory = asyncHandler(async (req, res, next) => {
  ...
  if (!req.files || !req.files.imageUrl) {
    return next(new ErrorResponse(STATUS_CODES.BAD_REQUEST, CATEGORY_ERRORS.NO_FILE_FOUND));
  }
  ...
  try {
    ...
    next(new ErrorResponse(STATUS_CODES.SERVER_ERROR, 'Category creation failed'));
  }
});
...
const getAllCategories = asyncHandler(async (req, res, next) => {
  try {
    ...
    return next(
      new ErrorResponse(STATUS_CODES.SERVER_ERROR, CATEGORY_ERRORS.CATEGORIES_FETCH_ERROR)
    );
  }
});
...
const getCategoryById = asyncHandler(async (req, res, next) => {
  try {
    ...
    if (!category) {
      return next(
        new ErrorResponse(STATUS_CODES.NOT_FOUND, CATEGORY_ERRORS.CATEGORY_NOT_FOUND)
      );
    }
    ...
    return next(
      new ErrorResponse(STATUS_CODES.SERVER_ERROR, CATEGORY_ERRORS.CATEGORY_FETCH_ERROR)
    );
  }
});
...
const updateCategory = asyncHandler(async (req, res, next) => {
  try {
    ...
    logger.debug(`Uploaded File: ${req.file ? req.file.originalname : 'No file uploaded'}`);
    ...
    if (req.files.imageUrl) {
      ...
      logger.warn(`No file uploaded for category ID: ${id}. Skipping image update.`);
    }
    ...
    return next(
      new ErrorResponse(STATUS_CODES.SERVER_ERROR, CATEGORY_ERRORS.CATEGORY_UPDATE_ERROR)
    );
  }
});
