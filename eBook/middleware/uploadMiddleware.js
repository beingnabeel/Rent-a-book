...
const fileFilter = (req, file, cb) => {
  ...
  if (field === 'thumbnailUrl') {
    ...
    if (!allowedExtensions.ebookThumbnail.includes(fileExtension)) {
      return cb(new Error('Invalid file type for thumbnail. Only JPG, PNG, and JPEG are allowed.'));
    }
  } else if (field === 'pdfUrl') {
    // For eBook PDFs, allow PDF files only
    if (!allowedExtensions.ebookPdf.includes(fileExtension)) {
      return cb(new Error('Invalid file type for PDF. Only PDF files are allowed.'));
    }
  } else if (field === 'imageUrl') {
    // For category image upload
    if (!allowedExtensions.category.includes(fileExtension)) {
      return cb(new Error('Invalid file type for category image. Only JPG, PNG, and JPEG are allowed.'));
    }
  }
    ...
    for (const [field, files] of Object.entries(req.files)) {
      ...
      files.forEach((file) => {
        if (file.size > maxFileSize) {
          return next(new Error(`File size for ${field} exceeds the limit of ${maxFileSize / (1024 * 1024)}MB.`));
        }
      });
    }
  }
...
//   if (field === 'thumbnailUrl') {
//     // For category and eBook thumbnail, allow image files
//     if (!allowedExtensions.ebookThumbnail.includes(fileExtension)) {
//       return cb(new Error('Invalid file type for thumbnail. Only JPG, PNG, and JPEG are allowed.'));
//     }
//   } else if (field === 'pdfUrl') {
